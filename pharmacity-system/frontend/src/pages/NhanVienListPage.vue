...
const selectedNhanVien = computed(() =>
  nhanViens.value.find((item) => item.id_nhan_vien === selectedId.value) || null
);

const isEditing = computed(() => Boolean(selectedNhanVien.value));

const metrics = computed(() => {
  const total = nhanViens.value.length;
  const active = nhanViens.value.filter((item) => item.trang_thai === "active").length;
  const inactive = total - active;
  const admins = nhanViens.value.filter((item) => item.vai_tro?.ten_vai_tro === "admin").length;
...
function applySelectedToForm(nhanVien) {
  form.ten_dang_nhap = nhanVien.ten_dang_nhap || "";
  form.ho_ten = nhanVien.ho_ten || "";
  form.id_vai_tro = nhanVien.id_vai_tro ? String(nhanVien.id_vai_tro) : "";
  form.id_bang_cap = nhanVien.id_bang_cap ? String(nhanVien.id_bang_cap) : "";
  form.trang_thai = nhanVien.trang_thai || "active";
  form.mat_khau = "";
  form.mat_khau_confirmation = "";
}
...
function selectNhanVien(nhanVien) {
  selectedId.value = nhanVien.id_nhan_vien;
  applySelectedToForm(nhanVien);
  passwordForm.new_password = "";
  passwordForm.new_password_confirmation = "";
  message.value = `Da nap nhan vien ${nhanVien.ho_ten} vao form.`;
  error.value = "";
}

function startCreate() {
  resetForm();
  message.value = "Dang o che do tao moi nhan vien.";
  error.value = "";
}
...
async function loadMeta() {
  ...
  try {
    ...
    vaiTros.value = vaiTroData;
    bangCaps.value = bangCapData;
  } finally {
    loading.meta = false;
  }
}
...
async function loadNhanViens() {
  ...
  try {
    nhanViens.value = await getNhanViens();

    if (selectedId.value) {
      const refreshed = nhanViens.value.find((item) => item.id_nhan_vien === selectedId.value);

      if (refreshed) {
        applySelectedToForm(refreshed);
      } else {
        resetForm();
      }
    }
...
async function handleSearch() {
  ...
  try {
    nhanViens.value = await searchNhanViens(keyword.value);
    message.value = `Tim thay ${nhanViens.value.length} nhan vien phu hop.`;
  } catch (err) {
    error.value = normalizeError(err);
  } finally {
...
async function handleSubmit() {
  ...
  try {
    if (isEditing.value) {
      await updateNhanVien(selectedId.value, {
        ten_dang_nhap: form.ten_dang_nhap,
        ho_ten: form.ho_ten,
        id_vai_tro: Number(form.id_vai_tro),
        id_bang_cap: Number(form.id_bang_cap),
        trang_thai: form.trang_thai,
      });

      message.value = "Cap nhat nhan vien thanh cong.";
    } else {
      await createNhanVien({
        ten_dang_nhap: form.ten_dang_nhap,
        mat_khau: form.mat_khau,
        mat_khau_confirmation: form.mat_khau_confirmation,
        ho_ten: form.ho_ten,
        id_vai_tro: Number(form.id_vai_tro),
        id_bang_cap: Number(form.id_bang_cap),
        trang_thai: form.trang_thai,
      ...
      message.value = "Tao nhan vien thanh cong.";
      resetForm();
    }
...
async function handleDelete(nhanVien) {
  ...
  loading.deleteId = nhanVien.id_nhan_vien;
  error.value = "";

  try {
    await deleteNhanVien(nhanVien.id_nhan_vien);

    if (selectedId.value === nhanVien.id_nhan_vien) {
      resetForm();
    }

    message.value = `Da xoa nhan vien ${nhanVien.ho_ten}.`;
    await loadNhanViens();
  } catch (err) {
    error.value = normalizeError(err);
  } finally {
...
async function handleChangePassword() {
  if (!selectedNhanVien.value) {
    error.value = "Hay chon nhan vien truoc khi doi mat khau.";
    return;
  }
  ...
  try {
    await changeNhanVienPassword(selectedNhanVien.value.id_nhan_vien, {
    ...
    passwordForm.new_password = "";
    passwordForm.new_password_confirmation = "";
    message.value = `Da doi mat khau cho ${selectedNhanVien.value.ho_ten}.`;
  } catch (err) {
    error.value = normalizeError(err);
  } finally {
...
onMounted(async () => {
  try {
    await Promise.all([loadMeta(), loadNhanViens()]);
  } catch (err) {
    error.value = normalizeError(err);
  }
});
