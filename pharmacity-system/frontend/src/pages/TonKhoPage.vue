...
function lotBadgeClass(lo) {
  const status = lotStatus(lo);
  if (status === "Het ton") return "soft-badge--orange";
  if (status === "Sap het han") return "soft-badge--blue";
  return "soft-badge--teal";
}

const thuocRows = computed(() => {
  return thuocs.value
    .map((thuoc) => {
      const relatedLots = loThuocs.value.filter((lo) => lo.id_thuoc === thuoc.ma_thuoc);
...
const selectedThuocLots = computed(() => {
  if (!selectedThuoc.value) return [];

  return loThuocs.value
    .filter((lo) => lo.id_thuoc === selectedThuoc.value.ma_thuoc)
...
function selectThuoc(thuoc) {
  ...
  message.value = `Dang hien ${selectedThuocLots.value.length} lo cua ${thuoc.ten_thuoc}.`;
  error.value = "";
}
...
async function loadInventory() {
  ...
  try {
    ...
    message.value = `Da dong bo ${thuocs.value.length} thuoc va ${loThuocs.value.length} lo thuoc.`;
  } catch (err) {
...
async function handleSearch() {
  ...
  try {
    const [thuocData, loData] = await Promise.all([searchThuocs(keyword.value), searchLoThuocs(keyword.value)]);
    thuocs.value = thuocData;
    loThuocs.value = loData;
    selectedThuoc.value = null;
    message.value = `Tim thay ${thuocs.value.length} thuoc va ${loThuocs.value.length} lo phu hop.`;
  } catch (err) {
    error.value = normalizeError(err);
  } finally {
