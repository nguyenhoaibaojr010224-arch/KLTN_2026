...
export default {
  ...
  computed: {
    ...
    currentRole() {
      return this.currentUser?.vai_tro?.ten_vai_tro || "Chua xac dinh";
    },
  },
  methods: {
    normalizeError(err) {
    ...
    async handleLogin() {
      ...
      try {
        ...
        this.message = response.message || "Dang nhap thanh cong.";
      } catch (err) {
    ...
    async loadNhanViens() {
      ...
      try {
        ...
        this.nhanViens = Array.isArray(response) ? response : response.data || [];
    ...
    handleLogout() {
      ...
      this.nhanViens = [];
      this.message = "Da xoa token khoi frontend.";
      this.error = "";
    },
  },
  mounted() {
    if (getAccessToken()) {
      this.message = "Frontend da phat hien token da luu trong localStorage.";
    }
  },
};
</script>
